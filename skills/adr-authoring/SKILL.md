...
- **Numbers are provisional until landed on the shared mainline.** A local
  commit does not reserve a number: under held-push rhythms another session can
...
The ADR file itself stays a durable `docs/adr/` doc — committed by explicit path
and pushed or held per the session's authority rhythm and push rules; it is **not**
