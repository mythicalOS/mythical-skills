...
codex exec --sandbox read-only --skip-git-repo-check -c model_reasoning_effort="xhigh" '<adversarial consult naming the artefact reference(s) + its cited evidence; ask for SEVERITY | file:line | issue | fix, or CLEAN>' < /dev/null > "$OUT" 2>"$OUT.log"
RC=$?   # capture it on the very next line — anything else clobbers $?
```

**`$RC` is half the verdict, and `"$OUT"` is only the other half when `$RC` is 0.**
A non-zero exit is a FAILED gate whatever `"$OUT"` contains — never a CLEAN
reading, and never "no findings". The failure modes are not hypothetical and they
all land in the reassuring direction, because an absent finding reads exactly like
a clean one:

- **The run was refused.** A consult whose prose reads as offensive tooling exits
  non-zero with an EMPTY `"$OUT"` and a log ending
  `ERROR: This content was flagged for possible cybersecurity risk`. That is a
  prompt-framing problem, not a verdict and not a defect in the reviewed work —
  reframe and retry once; do not record it as CLEAN and do not change the artefact
  in response.
- **The run was killed or timed out.** `"$OUT"` is empty or truncated mid-sentence
  while the log is large. A 0-byte `"$OUT"` is NOT by itself proof of death,
  though — both tools block-buffer redirected output, so check for a live process
  AND read the log tail before concluding anything.
- **The scope was too large to finish.** Split the review scope and re-run; a
  partial read is not a pass.

So: `[ "$RC" -eq 0 ]` and a `"$OUT"` that actually ends in the contracted verdict
line, or the gate did not run. Read `"$OUT.log"` before deciding which failure it
was. This is the same rule the daemon route states below for its `status` field —
one principle, two transports.
...
```bash
OUT=$(mktemp "${TMPDIR:-/tmp}/xmodel-<slug>.XXXXXX")   # this binding defines its own — the
                                                       # Claude-side block above is the OTHER
                                                       # procedure, not a preceding step
claude -p '<adversarial consult; ask for SEVERITY | file:line | issue | fix, or CLEAN>' --output-format text > "$OUT" 2>"$OUT.log"
RC=$?   # capture it on the very next line — anything else clobbers $?
```

Read-only consult, not a diff review. Same whole-file preference for playbook /
cross-file-convention edits. **The exit-status rule from the Claude-side binding
binds here identically:** a non-zero `$RC` is a FAILED gate whatever `"$OUT"`
contains, an empty `"$OUT"` is never "no findings", and a refusal or a kill is a
run to diagnose and retry — not a verdict to record.
