...
description: >-
  ...
  role/user/dispatcher instructions decide whether to act, write, commit, push,
  route, spawn, stop, or change scope.
assumes:
  - |
...
says which. Both are the same artefact under the same contract — consume and
verify them identically.
...
- **Authority stays external.** This skill restores context; it does not grant
  permission to continue, push, route, spawn, merge, stop, or widen scope.
...
Read each selected handoff fully. Note its `Authored` provenance line. Extract
the sections by name (the canonical list lives in `agent:good-night`
§"Handoff template"). Then follow its `Reading Order` exactly until you can
state the live thread. Stop there unless verification requires a cited file or
command.
...
- Is any dirty, local-only, uncommitted, or unpushed state still present and
  relevant?
...
- Ignoring dirty, uncommitted, unpushed, or local-only state named by the
  handoff.
