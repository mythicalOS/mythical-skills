...
STOP. No merge-close-out from this session; merge-close-out fires after fresh-<recipient> completes.
```
...
5. **The merge-close-out for the original dispatch fires when the fresh session completes the work** — NOT when the WIP-handoff lands. The cycle is not closed until the merged code ships. Track in the lead's status block: cycle is mid-stream, not closed.
...
- **Merge close-out** (`merge_closeout` kind): irreversible action landed. WIP-handoff does NOT trigger a merge-close-out from the same session.
