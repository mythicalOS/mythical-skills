...
- If coverage is thin in the area being refactored, **STOP** before proceeding with the structural change and chat-message the lead asking for the baseline-test decision. Propose the baseline-test scope; do NOT aim for "full coverage" — just enough that Step 1.2 (post-refactor test suite) has a meaningful parity claim to make. Wait for the lead's response before touching the structural surface.
