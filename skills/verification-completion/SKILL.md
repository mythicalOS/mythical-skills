...
| Dispatched work landed | The landing request's own `status` came back `landed` | The session reported "success"; a branch SHA; a diff — a published branch is not a landing |
...
- About to commit / publish a branch / report a branch ready / close out without fresh
  verification.
...
committing, publishing a branch, or reporting a branch ready for the merge gate; writing a
