...
- **The authority rhythm gates the irreversibles, and it is the invoking
  ...
  post-close-out branch publication + review-ready sequence runs continuously
  (rhythm B) is decided in the brief, not here (`ROLES.md` §"Authority rhythms").
  The landing is the lead's regardless — not part of this worker sequence
...
- **Rhythm STOP.** Under rhythm A the close-out is a STOP point — write it (it
  reports the branch + immutable SHA), await the green-light, then publish the
  branch with `git.push_branch` per `branch-lifecycle`. Under B/D-semi-auto the
  sequence is continuous; under C it queues for the cycle batch. The landing is
  the lead's — you never request one (`branch-lifecycle` §"Merge to main").
...
- Report `Branch: <name> @ <SHA>` (the immutable `rev-parse` SHA) in the close-out
  routed to the lead, and publish the branch with `git.push_branch` per the rhythm
  — per the canonical contract in `branch-lifecycle` §"Report the branch in the
  close-out": the close-out reports the SHA; the branch is review-ready once that
  SHA is on the remote (immediate under B/D-semi-auto, after the rhythm-gated
  publication under A/C). Do not imply publication-before-report — the reported SHA
  is captured at close-out, the publication may follow under A/C.
- The gate roles review against that SHA once it is on the remote; the lead owns
  the landing. You do not call `git.request_landing` and you never merge `main`.
