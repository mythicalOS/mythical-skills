...
description: |
  ...
  COORDINATION flow (naming, push, report, fetch, merge) lives in
...
— naming the feature branch, pushing it, reporting its SHA, gate-role fetch, and
the merge — is `branch-lifecycle`; this skill does not duplicate it.
...
- **Order with branch deletion.** The merged remote branch deletion is part of the
  lead's cleanup in `branch-lifecycle`; remove the worktree before deleting a
  branch the worktree still references.
...
- Does NOT name, push, or report the feature branch, and does NOT run gate-role
  fetch or the merge — that is `branch-lifecycle`.
