...
4. **A placement map** — for each internal artefact kind, the one directory it lives in. The rule
   this map exists to close: *work performed inside a sub-repository still writes its durable
   internal documents on the internal side.* Dispatch location does not change document
   ...
   **The map covers durable documents only — plans, PRDs, ADRs, retros, long-form review
   reports, glossaries.** It does **not** cover the coordination record kinds (handoff,
   dispatch, acknowledgment, task, closeout, merge_closeout, clarification, addendum,
   wip_handoff, risk_triage, design_review, test_strategy, code_review): those are not files,
   no session writes them to a tree, and giving one of them a directory in this map is the
   file-era mistake this whole substrate replaced. If a kind on that list appears in your
   placement map, the map is wrong — delete the row, do not create the directory.
...
verifies published refs, and a shared default branch moves underneath it.
...
**Output:** one **published** branch per repository — published with `git.push_branch`, never with
keystrokes of your own — each with a recorded sha, a zero-hit scan over its final tree and
messages, and a clean cross-model verdict.
...
- **Work from the PUBLISHED refs.** Require the delivery to name, per repository, the **remote, the
  ...
  **Fail closed at both steps.** An unguarded `git fetch` that fails leaves behind whatever
  ...
  checkout or a local branch name: either can hold uncommitted, unpublished or stale state that no
...
- Does NOT grant branch-publication, landing or history-rewrite authority; those stay with the invoking role's
  contract.
