...
3. Run the content checks locally:
   ...
   CI runs the same checks plus a vocabulary denylist gate
   (`scripts/check-denylist.py`) over every tracked file, and runs each script's
   `--selftest` first. All must pass. The push-form gate is the one most likely
   to surprise you: a skill may not spell a remote-mutating VCS or forge command
   at all — agents publish a branch with `git.push_branch` and request a landing
   with `git.request_landing`, and the daemon does the rest.
